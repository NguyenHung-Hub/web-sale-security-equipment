...
public interface ProductRepository extends JpaRepository<Product, String> {
    ...
    @Query(value = "select p.*, if(sum(order_items.quantity) = (SELECT sum(oi.quantity) FROM (orders o) join (order_items oi) on oi.order_id = o.order_id " +
            " where oi.product_id  = p.product_id and o.order_status= 'COMPLETED'),sum(order_items.quantity) , 0) as totalQuan from (order_items) right outer join (products p) on p.product_id =order_items.product_id join categories c on c.category_id = p.category_id join brands b on b.brand_id = p.brand_id " +
            "  where concat(p.title, c.name, b.name) like %?1% " +
            " group by p.product_id ", nativeQuery = true)
    public Page<Product> findByNameContaining(String name, Pageable pageable);

    @Query(value = "select p.*, if(sum(order_items.quantity) = (SELECT sum(oi.quantity) FROM (orders o) join (order_items oi) on oi.order_id = o.order_id " +
            " where oi.product_id  = p.product_id and o.order_status= 'COMPLETED'),sum(order_items.quantity) , 0) as totalQuan from (order_items) right outer join (products p) on p.product_id =order_items.product_id join product_reviews pr on pr.product_id=p.product_id " +
    ...
    @Query(value = "select p.*, if(sum(order_items.quantity) = (SELECT sum(oi.quantity) FROM (orders o) join (order_items oi) on oi.order_id = o.order_id " +
            " where oi.product_id  = p.product_id and o.order_status= 'COMPLETED'),sum(order_items.quantity) , 0) as totalQuan from (order_items) right outer join (products p) on p.product_id =order_items.product_id join (categories c) \n" +
    ...
    @Query(value = "select products.*, if(sum(order_items.quantity) = (SELECT sum(oi.quantity) FROM (orders o) join (order_items oi) on oi.order_id = o.order_id " +
            " where oi.product_id  = products.product_id and o.order_status= 'COMPLETED'),sum(order_items.quantity) , 0) as totalQuan from (order_items) right outer join (products) on products.product_id =order_items.product_id join categories c on c.category_id = products.category_id join brands b on b.brand_id = products.brand_id \n" +
